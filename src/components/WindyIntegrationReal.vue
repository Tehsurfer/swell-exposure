...
const initWindyMap = async () => {
  if (!windyMapContainer.value || !props.selectedSpot) return
  ...
  try {
    ...
    const options = {
      ...
      // Optional: What layer should be shown
      overlay: selectedDataType.value === 'wind' ? 'wind' : 'waves',
    }
    ...
    window.windyInit(options, (windyAPIInstance) => {
      ...
      // Mount the map to our container
      windyMapContainer.value.appendChild(map.getContainer())
...
const updateWindyData = async () => {
  ...
  try {
    if (windyAPI) {
      ...
      windyAPI.store.set('overlay', selectedDataType.value === 'wind' ? 'wind' : 'waves')
